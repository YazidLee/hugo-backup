...
2. 对Java的弱引用有所了解，不知道的可以看看之前的这篇文章 [ThreadLocal 分析（上）——Java中的引用](https://www.liyangjie.cn/posts/work/threadlocal-reference/)。
...
有没有似曾相识的感觉，在上一篇 [ThreadLocal 分析（上）——Java中的引用](https://www.liyangjie.cn/posts/work/threadlocal-reference/) 中，介绍过一个 `WeakHashMap`，它的 `Entry` 定义 `private static class Entry<K,V> extends WeakReference<Object> ...` 与这里的 `Entry` 如出一辙，第一段注释也写得很清楚，使用 `WeakReference` 作为Key是为了回收生命周期较长的大对象。
